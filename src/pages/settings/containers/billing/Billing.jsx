			...
			<div className='table table-borderless d-flex flex-column border p-3 w-100'>
				{invoices.filter(({ lines }) => lines.data.length).map(({ created, total, status, hosted_invoice_url, lines }, index) => (
					...
					>
						...
						<span>{dayjs.unix(created).format('DD MMM YYYY')}</span>
						<span>£{total / 100}</span>
	...
	return (
		<div ref={modalRef} className={containerClass}>
			<CancelPlan
				...
				onComplete={timestamp => setSuccessMessage(`Your subscription plan will cancel on ${dayjs.unix(timestamp).calendar()}`)}
			/>
			<ChangePlan
				show={changeModal}
